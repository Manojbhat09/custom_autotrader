...
# Define the CustomCallback class
class CustomCallback(BaseCallback):
    def __init__(self, check_freq: int, log_dir: str, model_dir: str, results_dir: str, verbose=1):
    ...
    def _on_step(self) -> bool:
        if (self.n_calls + 1) % self.check_freq == 0:
            # Save model
            save_path = os.path.join(self.model_dir, f'model_step_{self.num_timesteps}')
            self.model.save(save_path)
            logging.info(f'Model saved to {save_path} at step {self.num_timesteps}')
        return True
...
def train_save_plot_model(agent, model_name, model_dir, plot_dir, log_dir, results_dir, env_train, total_timesteps=1000):
    ...
    check_freq = 1000  # Set the checkpoint frequency to your preference
...
def calculate_metrics(df_account_value):
    ...
    # Other metrics can be added here, such as maximum drawdown, volatility, etc.
...
# def plot_all_results_df(plot_dir, df_account_value_a2c, df_account_value_ddpg,
#                      df_account_value_td3, df_account_value_ppo, df_account_value_sac,
#                      MVO_result):
#     try:
...
# The function to train models with the custom callback
def train_model_with_callback(agent, model, model_name, model_dir, log_dir, results_dir, total_timesteps=1000):
    check_freq = 1000  # Set the checkpoint frequency to your preference
    callback = CustomCallback(
        check_freq=check_freq,
        log_dir=log_dir,
...
def main():
    ...
    if if_using_a2c:
        ...
        trained_a2c = train_model_with_callback(agent, model_a2c, "a2c", model_dir, log_dir, results_dir)
        trained_a2c.save(os.path.join(model_dir, 'model_a2c'))  # save the final model
        trained_models.update({'a2c': trained_a2c})

    if if_using_ddpg:
        model_ddpg = agent.get_model("ddpg")
        trained_ddpg = train_model_with_callback(agent, model_ddpg, "ddpg", model_dir, log_dir, results_dir)
    ...
    # trained_models = train_save_plot_model(agent, env_train, model_dir, env_kwargs)
