...
def calculate_metrics(df_account_value):
    ...
    start_value = df_account_value['account_value'].iloc[0]
    end_value = df_account_value['account_value'].iloc[-1]
...
def update_metrics_table(model_name, metrics, file_path='metrics_table.csv', root_path=None):
    """
    Update the metrics table with the metrics of the given model.

    Args:
        model_name (str): The name of the model.
        metrics (dict): The performance metrics of the model.
        file_path (str): The path to the CSV file where the metrics table is stored.

    Returns:
        pd.DataFrame: The updated metrics table.
    """
    if root_path:
        file_path= os.path.join(root_path, file_path)
    metrics_df = pd.DataFrame(metrics, index=[model_name])
    if os.path.exists(file_path):
        existing_metrics_df = pd.read_csv(file_path, index_col=0)
        updated_metrics_df = pd.concat([existing_metrics_df, metrics_df]).drop_duplicates()
    else:
        updated_metrics_df = metrics_df
    
    updated_metrics_df.to_csv(file_path)
    return updated_metrics_df

def evaluate_single_model(trained_model, model_name, e_trade_gym, MVO_result, plot_dir):
    try:
        ...
        # Save results
        os.makedirs(os.path.join(plot_dir, model_name), exist_ok=True)
        ...
        plot_account_values_comparison(model_name, df_account_value, MVO_result, os.path.join(plot_dir, model_name))
        # Calculate performance metrics here (e.g., Sharpe ratio, CAGR, etc.)
        metrics = calculate_metrics(df_account_value)

        plot_metrics_over_time(df_account_value, os.path.join(plot_dir, model_name))
...
def plot_account_values_comparison(model_name, df_account_value, MVO_result, plot_dir):
    ...
    dfarr = np.array(df_account_value)[1:, 1][:, np.newaxis]
    mvo = np.array(MVO_result)
    df_comparison = np.concatenate([dfarr, mvo], axis=1)
...
def main():
    ...
    if if_using_a2c:
        ...
        metrics_a2c = evaluate_single_model(trained_a2c, 'a2c', e_trade_gym, MVO_result, plot_dir)
        update_metrics_table('a2c', metrics_a2c, root_path=results_dir)
    ...
    if if_using_ddpg:
        ...
        metrics_ddpg = evaluate_single_model(trained_ddpg, 'ddpg', e_trade_gym, MVO_result, plot_dir)
        update_metrics_table('ddpg', metrics_ddpg, root_path=results_dir)
    ...
    if if_using_ppo:
        ...
        metrics_ppo = evaluate_single_model(trained_ppo, 'ppo', e_trade_gym, MVO_result, plot_dir)
        update_metrics_table('ppo', metrics_ppo, root_path=results_dir)
    ...
    if if_using_td3:
        ...
        metrics_td3 = evaluate_single_model(trained_td3, 'td3', e_trade_gym, MVO_result, plot_dir)
        update_metrics_table('td3', metrics_td3, root_path=results_dir)
    # SAC Model
    if if_using_sac:
        SAC_PARAMS = {
            "batch_size": 128,
        ...
        metrics_sac = evaluate_single_model(trained_sac, 'sac', e_trade_gym, MVO_result, plot_dir)
        update_metrics_table('sac', metrics_sac, root_path=results_dir)
