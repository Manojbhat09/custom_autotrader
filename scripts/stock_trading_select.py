...
def main():
    ...
    trained_a2c = (
        agent.train_model(model=model_a2c, tb_log_name="a2c", total_timesteps=100)
        if if_using_a2c
        else None
    )
    import pdb; pdb.set_trace()
    trained_a2c.save(os.path.join(model_dir, 'model_a2c'))
    ...
    trained_ddpg = (
        agent.train_model(model=model_ddpg, tb_log_name="ddpg", total_timesteps=100)
        if if_using_ddpg
        else None
    )
    trained_ddpg.save(os.path.join(model_dir, 'model_ddpg'))
    ...
    trained_ppo = (
        agent.train_model(model=model_ppo, tb_log_name="ppo", total_timesteps=100) #TODO
        if if_using_ppo
        else None
    )
    ...
    trained_td3 = (
        agent.train_model(model=model_td3, tb_log_name="td3", total_timesteps=500000)
        if if_using_td3
        else None
    )
    ...
    trained_sac = (
        agent.train_model(model=model_sac, tb_log_name="sac", total_timesteps=100)
        if if_using_sac
        else None
    )
    ...
    # ax = result.plot()
    # fig = ax.get_figure()
    # fig.savefig('plot4.png')  # This will save the plot to a file named 'plot.png' in the current directory
    ...
    result2 = pd.merge(df_account_value_td3, MVO_result, left_index=True, right_index=True)
    result2.colums = ["td3", "mean var"]

    plot_path = os.path.join(plot_dir, 'td3_MVO.png')
    plt.rcParams["figure.figsize"] = (15, 5)
    plt.figure()
    ax = result2.plot()
    ax.legend(title='Legend', bbox_to_anchor=(1.05, 1), loc='upper left')
