...
if __name__ == "__main__":
    ...
    # Single Feature Forecasting (Closing Price)
    X_train_close, y_train_close = create_sequences(train[:, 1], seq_length)
    X_test_close, y_test_close = create_sequences(test[:, 1], seq_length)
    ...
    checkpoint_path = f'models/checkpoints/TSLA_checkpoint_{current_datetime}.pth'
    ...
    pattern = r'TSLA_checkpoint_\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.pth'
    ...
    plt.plot(range(0, len(predictions)), predictions, 'ro', label='Predicted Close Price')
    plt.plot(range(len(y_test_close)), y_test_close, 'g-', label='Actual Close Price')
    plt.legend()
